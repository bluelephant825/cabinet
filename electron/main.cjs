...
// Window geometry for the Chromium sidecar bounds-sync: the renderer
// (browser-view.tsx) positions the daemon-managed Chrome window over the
// browse pane, so it needs live content-bounds + focus/visibility state.
function windowGeometryPayload(win) {
  return {
...
ipcMain.handle("cabinet:get-window-geometry", (event) => {
  const win = BrowserWindow.fromWebContents(event.sender);
  if (!win || win.isDestroyed()) return { ok: false };
  return { ok: true, ...windowGeometryPayload(win) };
});

// The sidecar Chromium window cannot be reliably hidden without Automation
// permission, so when browse mode leaves the sidecar engine we pull the
// Cabinet window forward to cover it instead.
ipcMain.handle("cabinet:focus-app-window", (event) => {
  const win = BrowserWindow.fromWebContents(event.sender) ?? liveMainWindow();
  if (!win || win.isDestroyed()) return { ok: false };
...
function buildBrowserWindow() {
  ...
  // Push geometry/focus changes so the renderer can bounds-sync the Chromium
  // sidecar window over the browse pane.
  const sendGeometry = () => {
    if (!win.isDestroyed()) {
      win.webContents.send("cabinet:window-geometry", windowGeometryPayload(win));
    }
  };
  for (const evt of [
    "move",
    ...
    win.on(evt, sendGeometry);
  }
  win.webContents.on("did-finish-load", sendGeometry);
  return win;
}
