...
server/
  ...
  browser/                   → Chromium sidecar: Chrome for Testing over CDP, extensions, /browser/* routes (docs/BROWSER.md)
...
20. **Cabinet Browser is a real Chromium sidecar** — browse mode and browser extensions run in a daemon-managed Chrome for Testing instance driven over CDP (see `docs/BROWSER.md` for the full spec: engine routing, window sync, `/browser/*` API, `cabinet-browser` CLI). Three rules: **never re-add Chrome API shims to Electron** (`electron-chrome-extensions` and `browser-preload.cjs` were deliberately deleted); all extension work goes through `server/browser/` (`Extensions.loadUnpacked`, never manifest patching); the renderer only ever talks to `/api/browser`, never to the daemon or CDP directly. The renderer also never touches `window.CabinetDesktop` directly — all host-privileged calls go through the `CabinetHost` seam in `src/lib/host/` (electron/chromium/web adapters; see `docs/CHROMIUM_HOST.md`). A `CABINET_BROWSER_HOST_MODE` daemon flag exists for the forthcoming Chromium fork that hosts the shell UI in-window.
...
Queue: `IngestionOperation` includes `consolidate`/`lint` (no source, route `linking → complete`); whole-wiki actions `consolidate`, `lint`, `reprocess-all` live on the workflow action handler. A drained batch auto-enqueues `consolidate` via the `batch-pending.json` marker.

Knowledge graph (`src/lib/llm-wiki/graph/`, builder `server/ingestion/wiki-graph.ts`): `wiki/graph.json` is a Cabinet-maintained projection of the Wiki (pages, sources, topics, claims; every edge carries `provenance`, `extractor`, `confidence`, optional evidence). It is rebuilt after every agent pass, restored if the agent touches it, committed with publications, served by `GET /api/llm-wiki/graph?path=` and rendered by `src/components/wiki/knowledge-graph-viewer.tsx` (Sigma.js, client-only). Explicit edges always win over inferred ones; a graph that fails `validateWikiGraph` is never written. Inferred entities/claims/edges come only from `graph/analyze.ts` (quote-validated, ids recomputed by Cabinet, per-page cache under `.cabinet-state/llm-wiki/graph/analysis/`) via the `graph` queue operation. Phase docs: `docs/LLM_WIKI_PHASE_*.md` (latest: 31).
