-- Operational intent, not a rebuildable knowledge/search cache.
CREATE TABLE llm_wiki_jobs (
  sequence INTEGER PRIMARY KEY AUTOINCREMENT,
  id TEXT NOT NULL UNIQUE,
  cabinet_id TEXT NOT NULL,
  room_path TEXT,
  source_id TEXT,
  operation TEXT NOT NULL CHECK(operation IN ('create', 'update', 'delete', 'reprocess')),
  input_json TEXT,
  content_hash TEXT,
  ...
  status TEXT NOT NULL CHECK(status IN ('queued', 'normalizing', 'classifying', 'promoting', 'compiling', 'reconciling', 'complete', 'failed', 'needs-review')),
  ...
  CHECK((status IN ('normalizing', 'classifying', 'promoting', 'compiling', 'reconciling')) = (lease_token IS NOT NULL)),
  CHECK(operation = 'create' OR source_id IS NOT NULL),
  CHECK((operation IN ('create', 'update') AND input_json IS NOT NULL AND content_hash IS NOT NULL AND source_version_id IS NULL)
    OR (operation = 'delete' AND input_json IS NULL AND content_hash IS NULL AND source_version_id IS NULL)
...
CREATE TABLE llm_wiki_attempts (
  job_id TEXT NOT NULL REFERENCES llm_wiki_jobs(id),
