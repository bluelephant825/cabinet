-- The "graph" operation (LLM knowledge-graph enrichment) is whole-wiki work
-- like consolidate/lint: no Source identity, a leased "linking" stage, then
-- complete. SQLite cannot alter CHECK constraints, so rebuild the table and
-- copy existing rows. llm_wiki_attempts is rebuilt too, child first: dropping
-- llm_wiki_jobs while attempts rows still reference it would record deferred
-- FK violations that fail this transaction at commit.
CREATE TABLE llm_wiki_jobs_new (
  ...
  room_path TEXT,
  source_id TEXT,
  operation TEXT NOT NULL CHECK(operation IN ('create', 'update', 'delete', 'reprocess', 'consolidate', 'lint', 'graph')),
  input_json TEXT,
  content_hash TEXT,
  ...
  CHECK(operation IN ('create', 'consolidate', 'lint', 'graph') OR source_id IS NOT NULL),
  CHECK((operation IN ('create', 'update') AND input_json IS NOT NULL AND content_hash IS NOT NULL AND source_version_id IS NULL)
    OR (operation IN ('delete', 'consolidate', 'lint', 'graph') AND input_json IS NULL AND content_hash IS NULL AND source_version_id IS NULL)
