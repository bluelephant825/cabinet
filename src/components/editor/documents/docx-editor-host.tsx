...
/**
 * Host side of the DOCX editor: opens a document session, mounts the
 * same-origin `/document-editor` iframe and drives the frame bridge. Falls
 * back to the read-only docx-preview viewer when the document isn't editable.
 */
...
export function DocxEditorHost({ path, fallback, onStatus, onNavigate }: Props) {
  const { t } = useLocale();
  ...
  useEffect(() => {
    ...
    void fetch("/api/documents/open", {
      ...
      .then((session) => {
        ...
        if (!session.capabilities.edit || session.format !== "docx") {
        ...
        iframe.src = `/document-editor#format=docx&path=${encodeURIComponent(path)}&channel=${channelRef.current}`;
        setMode("edit");
      })
      .catch((e: Error) => {
        if (cancelled) return;
