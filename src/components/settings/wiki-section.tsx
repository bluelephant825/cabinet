...
interface Status {
  ...
  jobs: { id: string; status: string; operation: string; sourceId: string | null; input?: { path: string } | null; error: string | null; updatedAt: string; agentWarnings?: string[]; graphWarnings?: string[]; graph?: { analyzed?: number; cached?: number; failedBatches?: number } | null }[];
  graph?: { generatedAt: string; jobId: string; stats: { nodes: number; edges: number }; warnings: string[] } | null;
  ...
  return <section className="space-y-4 border-t border-border pt-6" aria-label="LLM Wiki">
    ...
    {!status ? <p role="status" className="text-sm">Connecting to the Wiki service…</p> : !status.enabled ? <Button disabled={busy} onClick={() => void act({ action: "enable" })}>Enable LLM Wiki</Button> : <>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button variant="outline" disabled={busy || status.busy || !status.provider.available} onClick={() => void act({ action: "graph" })}>Rebuild knowledge graph</Button>
      ...
      <div className="flex flex-wrap items-center gap-3 text-sm"><span role="status">{connectionError ? "Progress unavailable" : activity}</span><Button size="sm" variant="outline" disabled={busy} onClick={() => void act({ action: status.running ? "pause" : "resume" })}>{status.running ? "Pause" : "Resume"}</Button><a className="underline" href={href(`${status.wikiPath}/index.md`)}>Open Wiki</a><a className="underline" href={href(`${status.wikiPath}/graph.json`)}>Open knowledge graph</a></div>
      {status.graph && <p className="text-xs text-muted-foreground">Knowledge graph: {status.graph.stats.nodes} nodes, {status.graph.stats.edges} edges, updated {new Date(status.graph.generatedAt).toLocaleString()}{(() => {
        const run = [...status.jobs].reverse().find((job) => job.operation === "graph" && job.graph && typeof job.graph.analyzed === "number");
        return run?.graph ? ` · analyzed ${run.graph.analyzed}, cached ${run.graph.cached ?? 0}, ${run.graph.failedBatches ?? 0} failed` : "";
      })()}</p>}
      ...
      {!!jobs.length && <div className="space-y-3 rounded-md border border-border p-3" aria-label="Wiki ingestion progress" role="region">
        ...
        <div aria-live="polite" className="space-y-2 text-sm">{activeJobs.map((job) => <div key={job.id}><p className="break-words font-medium">{jobTitle(job)}</p><p>{jobStages[job.status]}</p><p className="text-xs text-muted-foreground">Operation last updated at {new Date(job.updatedAt).toLocaleTimeString()}</p>{!!job.agentWarnings?.length && <details className="mt-1 text-xs text-muted-foreground"><summary>{job.agentWarnings.length} agent warnings</summary><ul>{job.agentWarnings.map((warning, index) => <li key={index}>{warning}</li>)}</ul></details>}{!!job.graphWarnings?.length && <details className="mt-1 text-xs text-muted-foreground"><summary>{job.graphWarnings.length} graph warnings</summary><ul>{job.graphWarnings.map((warning, index) => <li key={index}>graph: {warning}</li>)}</ul></details>}</div>)}</div>
